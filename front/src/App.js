...
function App() {
  ...
  const [isAuthenticated, setIsAuthenticated] = useState(localStorage.getItem("isAuthenticated") === "true");  // 로그인 상태 관리
  ...
  const fetchKtasData = async () => {
    try {
      ...
      setKtasData(result.data);  // KTAS 데이터 설정
        ...
        <Route
          ...
          element={
            isAuthenticated ? (  // 로그인 여부 확인
              ...
              <Navigate to="/login" />  // 로그인되지 않으면 로그인 페이지로 리디렉션
            )
          }
        />
      </Routes>
